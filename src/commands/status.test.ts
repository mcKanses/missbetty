...
describe('status command', () => {
  ...
  test('prints JSON with proxy and project details', () => {
    ...
    (fs.readFileSync as unknown as jest.Mock).mockImplementation((...args: unknown[]) => {
      const inputPath = normalizePath(String(args[0]));
      if (inputPath.endsWith('/.betty/dynamic/app.yml')) {
        return [
          'http:',
          '  routers:',
          '    app:',
    ...
    (execSync as unknown as jest.Mock).mockImplementation((...args: unknown[]) => {
      ...
      if (command === 'docker inspect betty-traefik') {
        return Buffer.from('[{"State":{"Running":true,"StartedAt":"2026-05-02T00:00:00.000Z"}}]');
      }
      if (command === 'docker ps --format {{.ID}}') {
        return Buffer.from('abc123\n');
      }
      if (command === 'docker inspect abc123') {
        return Buffer.from('[{"NetworkSettings":{"Networks":{"betty_proxy":{"IPAddress":"172.18.0.2"}}},"State":{"Status":"running","StartedAt":"2026-05-02T00:00:00.000Z"},"RestartCount":1}]');
      }
